...
input_tensor <- torch_tensor(feats, dtype = torch_float())

input_size <- ncol(input_tensor) 
print(input_size)

num_samples <- nrow(input_tensor)
output_size <- num_samples
print(output_size)

train_dl <- dataloader(input_tensor, batch_size = 32, shuffle = TRUE)

# Define a dense layer
dense_layer <- nn_linear(input_size, output_size)
