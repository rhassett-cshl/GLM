...
source(paste0(root_dir, '/scripts/main_ep_glm_functions.R'))
...
alphaj$lambda_alphaj <- alphaj$alpha * lambda

# all.x ensures all rows in gb are retained, even if there's no match
updated_data <- merge(gb, alphaj[c("ensembl_gene_id", "lambda_alphaj")], by = "ensembl_gene_id", all.x = TRUE)

updated_data$zeta <- expNdot$exp_power

write.csv(updated_data, "/home/rhassett/GLM/data/k562_epft_chr22_norm.csv", row.names = FALSE)
